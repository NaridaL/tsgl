...
// 	}
// })
var srcFiles = fs.readdirSync('src/demo').filter(function (f) { return f !== 'tsconfig.json'; });
var mdTable = '<!--- DEMO-TABLE-START --->\n' +
    '||||\n' +
    '--- | --- | ---\n' +
    srcFiles.map(function (fileName) {
        var name = path.parse(fileName).name;
        var fileContents = fs.readFileSync('./src/demo/' + fileName, 'utf8');
        console.log(fileName);
        var match = fileContents.match(/\/\*\*[\s\S]*?\*\//gm)[0];
        var doc = match.split(/\r?\n/).slice(1, match[0].length - 2).map(function (line) { return line.trim().substr(2).trim(); }).join('\n');
        console.log(doc);
...
readme = readme.replace(/<!--- DEMO-TABLE-START [^]* DEMO-TABLE-END --->/, mdTable);
